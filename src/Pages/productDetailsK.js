...
import product5 from './Image/kid5.jpg';
import product6 from './Image/kid6.jpg';
import product7 from './Image/kid7.jpg';
import product8 from './Image/kid8.jpg';
import product9 from './Image/kid9.jpg';
import product10 from './Image/kid10.jpg';
import product11 from './Image/kid11.jpg';
import product12 from './Image/kid12.jpg';

const ProductDetailK = () => {
  ...
  const productK = products.find((p) => p.id === parseInt(id));
  const [selectedImage, setSelectedImage] = useState(productK?.image);
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ ...productK, size: selectedSize });
    alert(`${productK.name} (Size: ${selectedSize}) added to your cart.`);
  };
    ...
    <div className="container mx-auto px-4 py-8  pt-24 h-screen overflow-hidden flex items-center">
      <div className="flex flex-wrap lg:flex-nowrap gap-8 w-full h-full">
        ...
        <div className="w-full lg:w-1/5 flex lg:flex-col gap-4 overflow-y-auto">
          {productK.thumbnails.map((thumbnail, index) => (
            <img
              key={index}
              src={thumbnail}
        ...
        <div className="w-full lg:w-2/5 overflow-y-auto">
          <h1 className="text-3xl font-bold text-gray-800 mb-4">{productK.name}</h1>
          <div className="flex items-center mb-2">
            <span className="text-2xl font-bold text-red-600">{productK.price}</span>
          </div>
          <p className="text-gray-600 mb-6">{productK.fullDescription}</p>
          ...
          <div className="text-gray-500">
            <p>
              <strong>Category:</strong> Kid's Clothing
            </p>
            <p>
